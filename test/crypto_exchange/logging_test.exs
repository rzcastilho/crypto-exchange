defmodule CryptoExchange.LoggingTest do
  ...
  describe "context management" do
    ...
    test "with_context/2 executes function with temporary context" do
      ...
      result = Logging.with_context(%{request_id: "temp-456"}, fn ->
  ...
  describe "performance timing" do
    test "with_timing/3 measures and logs execution time" do
      log_output = capture_log([level: :info], fn ->
        result = Logging.with_timing("Test operation", %{category: :performance}, fn ->
          Process.sleep(10)  # Small delay for timing
          "success"
        end)
  ...
  describe "operation tracking" do
    test "start_operation/2 returns completion function" do
      log_output = capture_log([level: :info], fn ->
        complete_fn = Logging.start_operation("Test operation", %{
          category: :trading,
          user_id: "dave"
        })
    ...
    test "auth_event/2 sanitizes sensitive information" do
      log_output = capture_log([level: :info], fn ->
        Logging.auth_event("User authenticated", %{
          user_id: "henry",
          api_key: "test_key",
          secret_key: "sensitive_secret",  # Should be filtered out
          password: "secret_password"      # Should be filtered out
        })
      end)
  ...
  describe "integration scenarios" do
    test "complete trading operation with context and timing" do
      log_output = capture_log([level: :info], fn ->
        ...
        result = Logging.with_timing("Process order placement", %{category: :trading}, fn ->
          # Log API request
