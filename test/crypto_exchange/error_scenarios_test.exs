defmodule CryptoExchange.ErrorScenariosTest do
  ...
  describe "trading error scenarios" do
    test "handles order placement errors with user-friendly messages" do
      ...
      {:error, enhanced_error} =
        ErrorHandler.handle_trading_error(:place_order, balance_error, context)
    ...
    test "handles invalid order parameters" do
      ...
      {:error, enhanced_error} =
        ErrorHandler.handle_trading_error(:place_order, invalid_symbol_error, context)
    ...
    test "handles rate limiting in trading operations" do
      ...
      {:error, enhanced_error} =
        ErrorHandler.handle_trading_error(:place_order, rate_limit_error, context)

      assert enhanced_error.category == :rate_limiting
      # Default per ErrorHandler
      assert enhanced_error.retryable == false
      assert enhanced_error.severity == :error
      assert length(enhanced_error.recovery_suggestions) > 0
    end
    ...
    test "handles market data errors gracefully" do
      ...
      {:error, enhanced_error} =
        ErrorHandler.handle_trading_error(:get_market_data, market_error, context)
  ...
  describe "integrated error scenarios" do
    test "complete trading workflow with multiple error conditions" do
      log_output =
        capture_log(fn ->
          ...
          operation_result =
            Logging.with_timing("Complete trading operation", %{category: :trading}, fn ->
              ...
              {:error, enhanced_error} =
                ErrorHandler.handle_trading_error(:place_order, api_error, %{
                  user_id: "error_scenario_user",
                  symbol: "BTCUSDT",
                  side: "BUY"
                })
          ...
          Task.async(fn ->
            ...
            case rem(i, 3) do
              ...
              2 ->
                ...
                {:error, _error} =
                  ErrorHandler.handle_trading_error(:place_order, error, %{user_id: user_id})
            end
    ...
    test "error recovery suggestions are contextually appropriate" do
      ...
      {:error, hft_result} =
        ErrorHandler.handle_trading_error(:place_order, rate_error, hft_context)
      ...
      {:error, regular_result} =
        ErrorHandler.handle_trading_error(:place_order, rate_error, regular_context)
  ...
  describe "edge cases and boundary conditions" do
    test "handles extremely large error messages" do
      # ~13KB message
      large_error_msg = String.duplicate("error details ", 1000)
    ...
    test "handles recursive error scenarios" do
      ...
      log_output =
        capture_log(fn ->
          ...
          invalid_metadata = %{
            # PIDs can't be serialized to JSON
            pid: self(),
            # Refs can't be serialized
            ref: make_ref(),
            category: :test
          }
